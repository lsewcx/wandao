...
public:
   ...
   *
   */
  void reset(void) { farmlandStep = FarmlandStep::None; }
   ...
   */
  bool farmlandDetection(TrackRecognition &track,
    ...
    case FarmlandStep::Enter: //[02] 开进进入
    {
      ...
      if (breakLeft > 0) {
        ...
        conesEdgeRight.push_back(
            track.pointsEdgeRight[track.pointsEdgeRight.size() - 1]);
      } else                                   // 初始左右锥桶搜索
      {
        ...
        {
          if (pointEdgeDet[i].y < COLSIMAGE / 2 &&
              pointEdgeDet[i].x > conesEdgeLeft[0].x)
            conesEdgeLeft[0] = pointEdgeDet[i];

          if (pointEdgeDet[i].y > COLSIMAGE / 2 &&
              pointEdgeDet[i].x > conesEdgeRight[0].x)
            conesEdgeRight[0] = pointEdgeDet[i];
        }

        if (abs(conesEdgeRight[0].y - conesEdgeLeft[0].y) > COLSIMAGE / 3 &&
            abs(conesEdgeRight[0].x - conesEdgeLeft[0].x) >
                ROWSIMAGE / 3)                          // 单边
        {
      ...
      if (conesEdgeLeft.size() >= conesEdgeRight.size() &&
          conesEdgeLeft.size() >= 2) // 左边补右边
      {
        vector<POINT> pointsRep;
        if (conesEdgeRight.size() > 0 && conesEdgeRight[0].x > ROWSIMAGE / 3) {
          pointsRep = predictEdgeRight(conesEdgeLeft, conesEdgeLeft[0],
                                       conesEdgeRight[0]);
          indexDebug = 1;
        } else {
          pointsRep =
              predictEdgeRight(conesEdgeLeft, conesEdgeLeft[0], POINT(0, 0));
          indexDebug = 2;
        }
        conesEdgeRight.clear();
      ...
      } else if (conesEdgeLeft.size() < conesEdgeRight.size() &&
                 conesEdgeRight.size() >= 2) // 右边补左边
      {
        vector<POINT> pointsRep;
        if (conesEdgeLeft.size() > 0 && conesEdgeLeft[0].x > ROWSIMAGE / 3) {
          pointsRep = predictEdgeLeft(conesEdgeLeft, conesEdgeLeft[0],
                                      conesEdgeRight[0]);
          indexDebug = 3;
        } else {
          pointsRep =
              predictEdgeLeft(conesEdgeLeft, POINT(0, 0), conesEdgeRight[0]);
          indexDebug = 4;
        }
        conesEdgeLeft.clear();
    ...
    case FarmlandStep::Cruise: //[03] 完全驶入农田
    {
      ...
      if (abs(conesEdgeRight[0].y - conesEdgeLeft[0].y) > COLSIMAGE / 3 &&
          abs(conesEdgeRight[0].x - conesEdgeLeft[0].x) > ROWSIMAGE / 3) // 单边
      {
        if (conesEdgeLeft[0].x > conesEdgeRight[0].x) // 左单边
        {
          conesEdgeRight.clear();
  ...
  void drawImage(TrackRecognition track, Mat &image) {
    // 绘制4象限分割线
    line(image, Point(0, image.rows / 2), Point(image.cols, image.rows / 2),
         Scalar(255, 255, 255), 1);
    line(image, Point(image.cols / 2, 0), Point(image.cols / 2, image.rows - 1),
         Scalar(255, 255, 255), 1);

    // 绘制锥桶坐标
    for (int i = 0; i < pointEdgeDet.size(); i++) {
      circle(image, Point(pointEdgeDet[i].y, pointEdgeDet[i].x), 4,
             Scalar(92, 92, 205), -1); // 锥桶坐标：红色
    }

    for (int i = 0; i < conesEdgeLeft.size(); i++)
      circle(image, Point(conesEdgeLeft[i].y, conesEdgeLeft[i].x), 3,
             Scalar(0, 255, 0), -1); // 锥桶左边缘：绿色
    for (int i = 0; i < conesEdgeRight.size(); i++)
      circle(image, Point(conesEdgeRight[i].y, conesEdgeRight[i].x), 3,
             Scalar(0, 255, 255), -1); // 锥桶右边缘：黄色
    ...
    circle(image, Point(pointCorn.y, pointCorn.x), 3, Scalar(8, 112, 247),
           -1); // 玉米坐标绘制：橙色
...
private:
    ...
    while (pointsCones.size() > 0) {
      ...
      for (int i = 0; i < pointsCones.size(); i++) // 搜索左边缘
      {
        if (conesEdgeLeft.size() > 0) {
          if (distanceForPoints(conesEdgeLeft[conesEdgeLeft.size() - 1],
                                pointsCones[i]) <
                  conesEdgeLeft[conesEdgeLeft.size() - 1].x / 1.1 &&
              pointsCones[i].x < conesEdgeLeft[conesEdgeLeft.size() - 1].x) {
            if (index < 0)
              index = i;
            else if (distanceForPoints(
                         pointsCones[i],
                         conesEdgeLeft[conesEdgeLeft.size() - 1]) <
                     distanceForPoints(pointsCones[index],
                                       conesEdgeLeft[conesEdgeLeft.size() - 1]))
              index = i;
          }
        } else
      ...
      for (int i = 0; i < pointsCones.size(); i++) // 搜索右边缘
      {
        if (conesEdgeRight.size() > 0) {
          if (distanceForPoints(conesEdgeRight[conesEdgeRight.size() - 1],
                                pointsCones[i]) <
                  conesEdgeRight[conesEdgeRight.size() - 1].x / 1.1 &&
              pointsCones[i].x < conesEdgeRight[conesEdgeRight.size() - 1].x) {
            if (index < 0)
              index = i;
            else if (distanceForPoints(
                         pointsCones[i],
                         conesEdgeRight[conesEdgeRight.size() - 1]) <
                     distanceForPoints(
                         pointsCones[index],
                         conesEdgeRight[conesEdgeRight.size() - 1]))
              index = i;
  ...
  void searchCorn(vector<PredictResult> predict) {
    ...
    for (int i = 0; i < predict.size(); i++) {
      if (predict[i].label == LABEL_CORN &&
          abs(predict[i].x + predict[i].width / 2 - COLSIMAGE / 2) <
              distance) // 玉米检测
      {
        corn = POINT(predict[i].y + predict[i].height,
                     predict[i].x + predict[i].width / 2);
   ...
   * @return vector<POINT>
   */
  vector<POINT> predictEdgeRight(vector<POINT> &pointsEdgeLeft, POINT pointL,
                                 POINT pointR) {
    ...
    if (pointR.x > 0 && pointR.y > 0) {
      ...
      Point2d rightIpm =
          ipm.homography(Point2d(pointR.y, pointR.x)); // 透视变换
    ...
    vector<POINT> input = {startPoint, midPoint, endPoint};
    vector<POINT> repair = Beziekr(0.05, input);
   ...
   * @return vector<POINT>
   */
  vector<POINT> predictEdgeLeft(vector<POINT> &pointsEdgeRight, POINT pointL,
                                POINT pointR) {
    int offset = 120; // 右边缘平移尺度
