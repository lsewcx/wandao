...
/* 图像处理 */
class image{
void process(cv::Mat &mat_origin, cv::Mat &mat_result, )
    ...
    */
    ...
    if (_is_result)
    {
        ...
        if (flag_cross == CROSS_IN)
        {
            ...
            // 远线
            for (int a = 0; a < far_rpts0s_num; a++)
            { // 边线等距采样 左 十字远线
            ...
            for (int a = 0; a < far_rpts1s_num; a++)
            { // 边线等距采样 右 十字远线
        ...
        else if (flag_yroad > YROAD_FOUND)
        {
            ...
            // 边线
            for (int a = 0; a < yroad_rptss_num; a++)
            { // 边线等距采样 泛行区边线
